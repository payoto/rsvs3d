...
void error(const char *message = "", const char *caller = "", const char *file = "", int line = 0,
 ...
 *                       `T::is_open()`
 */
template <class T> void CheckFStream(const T &file, const char *callerID, const std::string &fileName)
{
    if (!file.is_open())
    {
        std::string errstr;
...
template <class E> void error(const char *message, const char *caller, const char *file, int line, bool throwError)
{
    std::cerr << std::endl << "Error at: " << file << ":" << line << std::endl << "    " << caller << std::endl;
    if (throwError)
    {
        throw E(message);
    }
