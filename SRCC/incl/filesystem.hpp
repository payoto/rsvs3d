...
// test for c++17 which has its own filesystem header
			...
			#if (__GNUC__==8)
				#error gcc v8.1 does not currently support <filesystem> header on windows (BUG) use <boost/filesystem.hpp>. Alternatively force use of the header by adding flag "-DUSE_CSTD_FILESYSTEM" to your compile command.
			#elif (__GNUC__==7)
