	...
	for(ii=0; ii<ni; ++ii){
		if(triIn.pointtype[ii]==meshtypes::mesh){
			...
			veccoordvol[1+ii] = &(triRSVS.meshDep->verts.isearch(
				triIn.pointind[ii])->coord);
			veccoordvol[1+3+ii] = &(triRSVS.meshDep->verts.isearch(
				triIn.pointind[ii])->coord);
			...
			veccoordvol[1+ii] = (triRSVS.trivert.isearch(
				triIn.pointind[ii])->coord.retPtr());
			veccoordvol[1+ii+3] = (triRSVS.trivert.isearch(
				triIn.pointind[ii])->coord.retPtr());
		}
	}
