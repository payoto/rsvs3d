...
void tetgen::internal::MeshData2Tetgenio(const mesh &meshgeom, tetgen::io_safe &tetin, int facetOffset, int pointOffset,
...
void tetgen::internal::Mesh2Tetgenio(const mesh &meshgeom, const mesh &meshdomain, tetgen::io_safe &tetin, int numHoles)
{
    ...
    */
    ...
    tetgen::internal::MeshData2Tetgenio(meshdomain, tetin, meshgeom.surfs.size(), meshgeom.verts.size(), -1, {-1.0},
                                        {0.0}, 1);
}
...
void tetgen::internal::PointCurvature2Metric(std::vector<double> &vertCurvature, const tetgen::apiparam &inparam)
{
    ...
    auto lininterp = [&](double curv) -> double {
        return (inparam.surfedgelengths[1] - inparam.surfedgelengths[0]) / (deltaCurv) * (curv - minCurv) +
               inparam.surfedgelengths[0];
    };
...
void tetgen::input::RSVS2CFD(const snake &snakein, tetgen::io_safe &tetin, const tetgen::apiparam &tetgenParam)
{
    ...
    */
    ...
    double distBound = tetgenParam.edgelengths.size() > 0 ? tetgenParam.edgelengths[0] : tetgenParam.distanceTol;
    for (int i = 0; i < 3; ++i)
    {
        lowerB[i] -= distBound;
        upperB[i] += distBound;
    }
    meshdomain = BuildCutCellDomain(tetgenParam.lowerB, tetgenParam.upperB, lowerB, upperB,
                                    tetgenParam.edgelengths.size(), vertPerSubDomain);
    ...
    {
        tetin.SpecifyTetPointMetric(0, startPnt, {tetgenParam.surfedgelengths[0]});
    }
    else
    {
        auto vertCurvature = CalculateVertexCurvature(meshgeom, tetgenParam.curvatureSmoothing);
        DisplayVectorStatistics(vertCurvature);
        tetgen::internal::PointCurvature2Metric(vertCurvature, tetgenParam);
        tetin.SpecifyTetPointMetric(0, startPnt, vertCurvature);
        DisplayVectorStatistics(vertCurvature);
    }
    for (int i = 0; i < int(tetgenParam.edgelengths.size()); ++i)
    {
        tetin.SpecifyTetPointMetric(startPnt, vertPerSubDomain[i], {tetgenParam.edgelengths[i]});
...
void tetgen::input::RSVSGRIDS(const mesh &meshdomain, const mesh &meshboundary, tetgen::io_safe &tetin,
                              const tetgen::apiparam &tetgenParam)
{
    ...
    */
    ...
    tetin.SpecifyTetPointMetric(startPnt, startPnt + vertEdgeLength.size(), vertEdgeLength);
}
...
void tetgen::input::RSVSGRIDS(const mesh &meshdomain, tetgen::io_safe &tetin, const tetgen::apiparam &tetgenParam)
{
    ...
    tetgen::input::RSVSGRIDS(meshdomain, meshboundary, tetin, tetgenParam);
}

void tetgen::input::POINTGRIDS(const mesh &meshdomain, tetgen::io_safe &tetin, const tetgen::apiparam &tetgenParam,
                               bool generateVoroBound)
{
    /*
    Processes a snake object into a tetgen input object.
            ...
            for (int i = 0; i < 8; ++i)
            {
                for (int j = 0; j < 3; ++j)
                {
                    ...
                    vertsToAdd.push_back(
                        (((i / k) % 2) ? tetgenParam.upperB[j] + delta : tetgenParam.lowerB[j] - delta));
                }
                nVertsAdded++;
            }
        }
            ...
            { // for each direction of the edge (coord = 0.5)
                ...
                { // for each displacement off the edge
                    ...
                    { // each of the edge that make a square
                        ...
                        {
                            if (m == l)
                            { // this is the inactive dimension
                                vertsToAdd.push_back(edgeModif[l]);
                            }
                            else
                            {
                                double delta = (tetgenParam.upperB[m] - tetgenParam.lowerB[m]) * vertModif[m];
                                int k = pow(2, (m + 3 - (l + 1)) % 3);
                                vertsToAdd.push_back(
                                    (((i / k) % 2) ? tetgenParam.upperB[m] + delta : tetgenParam.lowerB[m] - delta));
                            }
                        }
                        nVertsAdded++;
                    }
...
void tetgen::output::SU2(const char *fileName, const tetgenio &tetout)
{
    ...
    if (meshFile.fail())
    {
        std::cerr << std::endl << "Error: " << fileName << std::endl << " in could not be opened" << std::endl;
...
void tetgen::internal::CloseVoronoiMesh(mesh &meshout, tetgen::io_safe &tetout, std::vector<int> &rayEdges, int DEINCR,
                                        tetgen::dombounds boundBox)
{
    ...
    */
    ...
    for (int i = 0; i < count; ++i)
    {
        ...
        lStep =
            tetgen::internal::ProjectRay(3, boundBox, tetout.vedgelist[rayEdges[i] - 1].vnormal,
                                         meshout.verts[tetout.vedgelist[rayEdges[i] - 1].v1 + DEINCR].coord, lStepMin);
    ...
    rayFaces = ConcatenateVectorField(meshout.edges, &edge::surfind, meshout.edges.find_list(rays.vec));
    sort(rayFaces);
    unique(rayFaces);
    count = rayFaces.size();
    ...
    for (int i = 0; i < count; ++i)
    {
        ...
        if (pair.size() != 2)
        {
            RSVS3D_ERROR_LOGIC("Voronoi open face should have 2 rays");
        }
    ...
    rayCells = ConcatenateVectorField(meshout.surfs, &surf::voluind, meshout.surfs.find_list(rayFaces));
    sort(rayCells);
    unique(rayCells);
    count = rayCells.size();
    ...
    for (int i = 0; i < count; ++i)
    {
        ...
        for (auto a : rayFacesSearch.find_list(meshout.volus[rayCells[i] - 1].surfind))
        {
            if (a != -1)
            {
                pair.push_back(a);
        ...
        for (auto ind : pair)
        {
            ...
            if (k > 1)
            {
                std::cerr << "Unexpected behaviour with number of edges" << std::endl;
            }
        }
...
tetgen::dombounds tetgen::output::GetBoundBox(tetgen::io_safe &tetout)
{
    ...
    for (int i = 0; i < count; ++i)
    {
        ...
        {
            domBounds[0][j] =
                domBounds[0][j] <= tetout.pointlist[i * n + j] ? domBounds[0][j] : tetout.pointlist[i * n + j];
            domBounds[1][j] =
                domBounds[1][j] >= tetout.pointlist[i * n + j] ? domBounds[1][j] : tetout.pointlist[i * n + j];
        }
    }
...
mesh tetgen::output::VORO2MESH(tetgen::io_safe &tetout)
{
    ...
    */
    ...
    { // Assign edge to vertex connectivity
        meshout.edges[i].vertind = {tetout.vedgelist[i].v1 + INCR, tetout.vedgelist[i].v2 + INCR};
    ...
    { // loop through surfs
        ...
        for (int j = 0; j < n; ++j)
        { // loop through corresponding connections
 ...
 * mesh volumes.
 */
std::vector<int> tetgen::RSVSVoronoiMesh(const std::vector<double> &vecPts, mesh &vosMesh, mesh &snakMesh,
                                         tetgen::apiparam &inparam)
{
    ...
    Step 3 - detail:
            ...
            4. Remove all points which are marked with block numbers to generate
    the parent mesh.
    ...
    */
    ...
    auto boundFaces = tetgen::voronoi::Points2VoroAndTetmesh(vecPts, voroMesh, snakMesh, inparam);
    std::cout << "done. ";
    ...
    {
        std::cerr << "Error : nBlocks (" << nBlocks << ")!=nVolus Voromesh (" << voroMesh.volus.size() << ")"
                  << std::endl;
    ...
    {
        std::cerr << "Error : vosMesh (" << vosMesh.volus.size() << ")!=nVolus Voromesh (" << voroMesh.volus.size()
                  << ")" << std::endl;
    ...
    {
        vosMesh
            .volus[vosMesh.volus.find(elmMapping[snakMesh.volus.find(vecPtsMapping[i])],
 ...
 * @param      inparam   tetgen interface parameter object. Used to define
 * boundary growth rate and element sizes.
 */
 ...
 *
 * @throw  invalid_argument if tetout was generated without passing the
 * neighbour flag to tetgen (`-nn`)
 */
mesh tetgen::output::TET2MESH(tetgen::io_safe &tetout)
{
    ...
    */
    ...
    for (int i = 0; i < count; ++i)
    { // Assign edge to vertex connectivity
        meshout.edges[i].vertind = {tetout.edgelist[i * 2] + INCR, tetout.edgelist[i * 2 + 1] + INCR};
        for (int j = 0; j < 2; ++j)
        { // Assign equivalent vertex to edge connectivity
            meshout.verts[tetout.edgelist[i * 2 + j] + DEINCR].edgeind.push_back(i + 1);
        }
    }
    std::cout << "done. ";
...
tetgenmesh *tetgen::rsvstetrahedralize(const char *switches, tetgen::io_safe *in, tetgen::io_safe *out,
                                       tetgen::io_safe *addin, tetgen::io_safe *bgmin)
{
    std::cout << std::endl << "__________________________________________________" << std::endl;
    std::cout << std::endl
              << " TetGen start (cmd: " << switches << ")" << std::endl
...
std::vector<bool> tetgen::voronoi::Points2VoroAndTetmesh(const std::vector<double> &vecPts, mesh &voroMesh,
                                                         mesh &tetMesh, const tetgen::apiparam &inparam)
{
    /*
...
void tetgen::io_safe::allocate()
{
    ...
    if (this->numberoftetrahedra > 0)
    {
        ...
        this->tetrahedronvolumelist = new REAL[this->numberoftetrahedra * this->numberoftetrahedronattributes];
    ...
    if (this->numberofvedges || this->numberofvpoints || this->numberofvcells || this->numberofvfacets)
    {
...
void tetgen::io_safe::allocatefacet(int fIndex)
{
    if (fIndex < this->numberoffacets)
    {
        ...
        {
            this->facetlist[fIndex].polygonlist = new tetgenio::polygon[this->facetlist[fIndex].numberofpolygons];
        }
        else
        {
        ...
        {
            this->facetlist[fIndex].holelist = new REAL[this->facetlist[fIndex].numberofholes];
        }
        else
        {
...
void tetgen::io_safe::allocatefacetpolygon(int fIndex, int pIndex)
{
    if (fIndex < this->numberoffacets)
    {
        if (pIndex < this->facetlist[fIndex].numberofpolygons)
        {
            ...
            {
                this->facetlist[fIndex].polygonlist[pIndex].vertexlist = (int *)NULL;
            }
        }
        else
        {
...
void tetgen::io_safe::SpecifyTetPointMetric(int startPnt, int numPnt, const std::vector<double> &mtrs)
{
    /*
    assigns the mtr list from `startPnt` index
    */

    int count = int(mtrs.size()) < this->numberofpointmtrs ? mtrs.size() : this->numberofpointmtrs;
    ...
    for (int i = startPnt; i < startPnt + numPnt; ++i)
    {
        ...
        {
            this->pointmtrlist[(i * this->numberofpointmtrs) + j] = mtrs[j];
        }
    }
}

void tetgen::io_safe::SpecifyIndividualTetPointMetric(int startPnt, int numPnt, const std::vector<double> &mtrs)
{
    /*
    assigns the mtr list from `startPnt` index
    */
    ...
    for (int i = 0; i < count; ++i)
    {
        this->pointmtrlist[(startPnt * this->numberofpointmtrs) + i] = mtrs[i];
    }
}
void tetgen::io_safe::SpecifyTetFacetMetric(int startPnt, int numPnt, int marker)
{
    /*
...
void tetgen::io_safe::displaypoints()
{
    ...
    {
        std::cout << "metric " << j << " :";
        for (int i = 0; i < this->numberofpoints * this->numberofpointmtrs; i = i + this->numberofpointmtrs)
        {
...
int tetgen::test::api()
{
    ...
    }
    {
        ...
        if (p1.edgelengths.size() != 3)
        {
            ...
            std::cerr << "edgelengths.size " << p1.edgelengths.size() << std::endl;
            errCount++;
        }
        else
        {
...
int tetgen::test::call()
{
    ...
    try
    {
        ...
        std::vector<int> subs;
        subs = ConcatenateVectorField(meshvoro.volus, &volu::surfind, 0, meshvoro.volus.size());
...
int tetgen::test::RSVSVORO()
{
    ...
    vector<int> numCells = {0, 1, 2, 3, 4, 5, 10, 20, 100, 1000};
    vector<double> numEdge = {0.05, 0.1, 0.3};
    // std :: cin >> nPts;
...
int tetgen::test::RSVSVOROFunc_contain(int nPts, double distanceTol, const char *tecoutStr)
{
    ...
    try
    {
        ...
        cout << "Start Voronoi mesh generation for " << nPts << " points" << endl;
        cout << "__________________________________________________" << endl;
        ...
        {
            meshPts.verts[i].coord.assign(vecPts.begin() + i * 4, vecPts.begin() + i * 4 + 3);
        }
        ...
        for (int i = 0; i < nVecMap; ++i)
        {
            ...
            tecout.PrintMesh(vosMesh, nPts * 3 + 1, i, 5, {snakMesh.ParentElementIndex(vecMap[i])});
            tecout.PrintMesh(snakMesh, nPts * 3 + 2, i, 5, {vecMap[i]});
            tecout.PrintMesh(meshPts, nPts * 3 + 3, i, rsvs3d::constants::tecplot::point, {i + 1});
        }
        cout << "__________________________________________________" << endl;
    }
    catch (exception const &ex)
...
int tetgen::test::RSVSVOROFunc(const std::vector<double> &vecPts, double distanceTol, const char *tecoutStr)
{
    // std::vector<double> vecPts;
    ...
    try
    {
        ...
        meshPts.PrepareForUse();
        tecout.PrintMesh(meshPts, 3, nPts + distanceTol, rsvs3d::constants::tecplot::point);
        cout << "__________________________________________________" << endl;
    }
    catch (exception const &ex)
    {
