...
void PlaceBorderVertex(const std::vector<double> &coordIn, 
	...
	const tetgen::dombounds &cropBox, std::vector<double> &coordTarg){

	for (int i = 0; i < 3; ++i)
	{
		coordTarg[i] = coordIn[i]-coordOut[i];
	}
	double lStep = tetgen::voronoi::ProjectRay(3, cropBox, 
...
void CropMesh(mesh &meshin, tetgen::dombounds &cropBox){
	/*
	Crops a mesh along a boundary ensuring closed elements.
	...
	*/
	...
	int vertSize, edgeSize, surfSize, voluSize, count;

	vertSize=meshin.verts.size();
	edgeSize=meshin.edges.size();
	surfSize=meshin.surfs.size();
	voluSize=meshin.volus.size();
	vertOut.assign(vertSize, false);
	edgeOut.assign(edgeSize, false);
	surfOut.assign(surfSize, false);
	...
	for (int i = 0; i < vertSize; ++i)
	{
		for (int j = 0; j < 3; ++j)
		{
			if(meshin.verts(i)->coord[j]>cropBox[1][j] 
				|| meshin.verts(i)->coord[j]<cropBox[0][j]){
				vertOut[i] = true;
				break;
			}
		}
	...
	{ // for each edge if it's vertices are one in one out.
		if(vertOut[meshin.verts.find(meshin.edges(i)->vertind[0])] 
			^ vertOut[meshin.verts.find(meshin.edges(i)->vertind[1])]){
			edgeBound.push_back(meshin.edges(i)->index);
		}
	}
	count = edgeBound.size();
	auto tempVec = meshin.edges.find_list(edgeBound);
	surfBound = ConcatenateVectorField(meshin.edges, &edge::surfind, tempVec);
	sort(surfBound);
	unique(surfBound);
	tempVec.clear();
	tempVec = meshin.edges.find_list(surfBound);
	voluBound = ConcatenateVectorField(meshin.surfs, &surf::voluind, tempVec);
	sort(voluBound);
	unique(voluBound);
	...
	meshin.MakeCompatible_inplace(meshAdd);
	meshin.Concatenate(meshAdd);
	meshin.HashArray();
	...
	count = edgeBound.size();
	tempVec = meshin.edges.find_list(edgeBound);
	for (int i = 0; i < count; ++i)
	{
		// Replicate edge (changing index)
		auto tempInd = meshin.edges[i+edgeSize].index;
		meshin.edges[i+edgeSize] = meshin.edges[tempVec[i]];
		meshin.edges[i+edgeSize].index = tempInd;
		// Compute vertex position
		int iIn=0, iOut=1;
		iIn += vertOut[meshin.verts.find(meshin.edges[tempVec[i]].vertind[0])];
		iOut -= vertOut[meshin.verts.find(meshin.edges[tempVec[i]].vertind[0])];

		PlaceBorderVertex(
			meshin.verts.isearch(meshin.edges[tempVec[i]].vertind[iIn])->coord, 
			meshin.verts.isearch(meshin.edges[tempVec[i]].vertind[iOut])->coord,
			cropBox, meshin.verts[i+vertSize].coord
			);
...
void CropMeshGreedy(mesh &meshin, tetgen::dombounds &cropBox){
	/*
	...
	*/
	...
	auto vertSub = meshin.verts.find_list(vertDel);
	auto edgeDel = ConcatenateVectorField(meshin.verts, &vert::edgeind, vertSub);
	auto edgeSub = meshin.edges.find_list(edgeDel);
	auto surfDel = ConcatenateVectorField(meshin.edges, &edge::surfind, edgeSub);
	auto surfSub = meshin.surfs.find_list(surfDel);
	auto voluDel = ConcatenateVectorField(meshin.surfs, &surf::voluind, surfSub);
	...
	count = voluDel.size();
	for (int i = 0; i < count; ++i)	{meshin.RemoveIndex(4, voluDel[i]);}

	// Expand surf removal list
	count = meshin.surfs.size();
	for (int i = 0; i < count; ++i)
	{
		if(meshin.surfs(i)->voluind.size()==0){
			surfDel.push_back(meshin.surfs(i)->index);
		} else if (meshin.surfs(i)->voluind.size()==1){
			meshin.surfs[i].voluind.push_back(0);
		}
	}
	...
	unique(surfDel);
	count = surfDel.size();
	for (int i = 0; i < count; ++i)	{meshin.RemoveIndex(3, surfDel[i]);}

	count = meshin.edges.size();
	for (int i = 0; i < count; ++i)
	{
		if(meshin.edges(i)->surfind.size()==0){
			edgeDel.push_back(meshin.edges(i)->index);
		} 
	}
	sort(edgeDel);
	unique(edgeDel);
	count = edgeDel.size();
	for (int i = 0; i < count; ++i)	{meshin.RemoveIndex(2, edgeDel[i]);}

	count = meshin.verts.size();
	for (int i = 0; i < count; ++i)
	{
		if(meshin.verts(i)->edgeind.size()==0){
			vertDel.push_back(meshin.verts(i)->index);
		} 
	}
	sort(vertDel);
	unique(vertDel);
	count = vertDel.size();
	for (int i = 0; i < count; ++i)	{meshin.RemoveIndex(1, vertDel[i]);}


	meshin.verts.remove(vertDel);
	meshin.edges.remove(edgeDel);
	meshin.surfs.remove(surfDel);
	meshin.volus.remove(voluDel);

	meshin.PrepareForUse();

	meshin.RemoveSingularConnectors();
...
namespace voronoimesh {
	...
	std::vector<int> Points2VoroAndTetmesh(const std::vector<double> &vecPts,
		...
		*/
		...
		CropMeshGreedy(voroMesh, cropBox);
